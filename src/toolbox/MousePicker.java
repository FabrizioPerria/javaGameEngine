...
public class MousePicker {
	...
	private TerrainSet terrains;
	private Vector3f currentTerrainPoint;

	public MousePicker(Camera cam, Matrix4f projection, TerrainSet terrains) {
		camera = cam;
	...
	private Vector2f getNormalisedDeviceCoordinates(float mouseX, float mouseY) {
		float x = (2.0f * mouseX) / DisplayManager.getWindowWidth() - 1f;
		float y = (2.0f * mouseY) / DisplayManager.getWindowHeight() - 1f;
	...
	private Terrain getTerrain(float worldX, float worldZ) {
		return terrains.getTerrainByCoord(worldX, worldZ);
	}
