...
public class ShadowBox {
	...
	protected void update() {
		...
		Vector4f forwardVector4 = new Vector4f(FORWARD);
		rotation.transform(forwardVector4,forwardVector4);
		Vector3f forwardVector = new Vector3f(forwardVector4.x, forwardVector4.y, forwardVector4.z);

		Vector3f toFar = forwardVector.mul(SHADOW_DISTANCE, new Vector3f());
		Vector3f toNear = forwardVector.mul(MasterRenderer.NEAR_PLANE, new Vector3f());
	...
	protected Vector3f getCenter() {
		...
		invertedLight.set(lightViewMatrix).invert();
		Vector4f center = new Vector4f();
		invertedLight.transform(cen, center);
	...
	// 	Vector3f upVector = new Vector3f(Matrix4f.transform(rotation, UP, new Vector3f()));
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
			Vector3f centerNear, Vector3f centerFar) {
		Vector4f upVector = new Vector4f(UP);
		...
		Vector3f downVector = new Vector3f(upVector.x, upVector.y, upVector.z).negate();
		Vector3f leftVector = new Vector3f(rightVector).negate();
		Vector3f farTop = new Vector3f(upVector.x, upVector.y, upVector.z).mul(farHeight).add(centerFar);
		Vector3f farBottom = new Vector3f(downVector).mul(farHeight).add(centerFar);
		Vector3f nearTop = new Vector3f(upVector.x, upVector.y, upVector.z).mul(nearHeight).add(centerNear);
